            ...
            ?>
                ...
                <?php if (get_next_posts_link()) : ?>
                    <div class="nav-previous"><span class="meta-nav">&larr;</span>
                        <?php next_posts_link(__('Older posts', 'knowhow')); ?></div>
                <?php endif; ?>
    ...
    if (1 != $pages) {
        ...
        for ($i = 1; $i <= $pages; $i++) {
            if (1 != $pages && (!($i >= $paged + $range + 1 || $i <= $paged - $range - 1) || $pages <= $showitems)) {
                ...
                if ($paged == $i) {
                    echo "<div class='current'><span>" . $i . "</span></div>";
                } else {
                    echo "<div class='page'><span><a href='" . get_pagenum_link($i) . "' class='inactive' >" . $i . "</a></span></div>";
                }
            }
        }
    ...
    if (!is_front_page()) {
        ...
        if (is_category()) {
            ...
            foreach ($terms as $term) {
                if ($term->parent != 0) {
                    echo get_category_parents($term->term_id, true, ' ' . $sep . ' ', true);
                } else {
